...
func (h *Hybrid) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, withdrawals []*types.Withdrawal) {
	engine := h.selectEngineFromHeader(header)
	engine.Finalize(chain, header, state, txs, uncles, withdrawals)
}
...
func (h *Hybrid) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, withdrawals []*types.Withdrawal) (*types.Block, error) {
	engine := h.selectEngineFromHeader(header)
	block, err := engine.FinalizeAndAssemble(chain, header, state, txs, uncles, receipts, withdrawals)
...
func (h *Hybrid) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	engine := h.selectEngine(0) // Use current engine for API exposure
